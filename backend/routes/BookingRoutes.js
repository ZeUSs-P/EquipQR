...
router.post("/", protect, async (req, res) => {
  ...
  // Deduct stock immediately when booking is created
  for (let i of items) {
    const dbItem = await Item.findById(i.item);
  ...
  // Set auto-delete timer (2 minutes)
  setTimeout(async () => {
    try {
...
router.get("/", async (req, res) => {
  const bookings = await Booking.find().populate("user", "name email").populate("items.item");
  res.json(bookings);
});
